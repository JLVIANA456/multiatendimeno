...
export default function ChatProvider(props: { children: any }) {
  ...
  const fetchInbox = useCallback(async () => {
    ...
    if (error) return console.error("Erro ao buscar conversas:", error);
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      setLoading(true);
      const lid = activeChat.id; // Busca pelo ID completo conforme salvo no webhook
      const { data, error } = await supabase
        .from("mensagens")
        .select("*")
        .eq("lid", lid)
      ...
      if (!error) {
        const mapped: Message[] = data.map((msg) => ({
          id: msg.message_id,
          body: msg.message || msg.body || msg.caption || "", // Tenta múltiplos campos por segurança
  ...
  }), [user, inboxState, activeChat, messages, loading, startOutboundChat, takeChat, handleChangeChat, clearMessages, deleteActiveChat, sendMessage, selectChatByPhone]);
